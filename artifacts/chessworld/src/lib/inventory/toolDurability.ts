...
/** Durabilidade máxima configurada para a ferramenta (padrão 100; cacheado por ref). */
...
/** Estado da barra para a UI (null = não é ferramenta ou máximo desconhecido). */
export function toolDurabilityView(
  itemKey: string | null,
  remaining: number | null | undefined,
  max: number | undefined,
): ToolDurabilityView | null {
  if (!itemKey || !isToolItemKey(itemKey) || typeof max !== 'number' || max < 1) return null;
