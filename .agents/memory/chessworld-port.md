...
- Decisões de jogo: energia NUNCA bloqueia ação (só fome/fraco/morte por limiares); custos "X a cada N" com contadores persistidos; craft cobra energia/XP × quantidade; comer consome só o necessário até encher (energia cheia → erro); revive enche energia; morte por fome usa `killPlayer(sessionId,'Fome')`; aba de cozinha da fornalha é reconhecida pelo NOME (Cozinhar/Cooking/Culinária via `isCookingTab`).
