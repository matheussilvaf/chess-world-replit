...
- Upload para bucket PÚBLICO (`craft-items`): o MIME do data URL é entrada do cliente — sempre validar magic bytes (png/jpeg/webp/gif) antes de subir, senão o bucket vira hospedagem de bytes arbitrários. createBucket lazy/idempotente; nome com timestamp (cache CDN) + limpeza dos antigos por prefixo.
- express.json global foi para 8mb por causa dos data URLs (limite por rota não funciona: o parser global roda antes e derruba com 413).
