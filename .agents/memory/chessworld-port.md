...
**How to apply:** use the tester only for non-Phaser routes (/admin, login, region select); verify in-world UI via tsc + targeted logic review + real-device testing by the user. **Update (set/2026):** HUD pieces built as plain React over zustand (inventory panel, hotbar) CAN be tested — the DEV-only bench route `/dev/inventario` (registered in main.tsx only under `import.meta.env.DEV`) seeds the stores without Phaser/Colyseus/login; prefer adding such a bench over declaring a HUD feature untestable. The tester's aria snapshot marks the FOCUSED element as `[active]` — not a "selected/equipped" state; assert on classes/titles instead. Related pattern: distinguishing live chat messages from history loads must use an explicit store signal (`liveChatMessage`, set only in addChatMessage, cleared in enterRegion) — length-diff heuristics ghost-fire on region re-entry because loadChat bulk-replaces the array.
...
## Combat: death/respawn + honest client cooldown (jul/2026)
- Ghost swings: o servidor DROPA silenciosamente `attack` antes de `duração+ATTACK_COOLDOWN_PAD_MS`; o cliente TEM que espelhar esse cooldown (`attackCooldownUntil = attackingUntil + PAD + MARGIN`) senão spam de F anima local sem dano. Consts compartilhadas em CharacterCombatShapes.ts (PAD 150, margem cliente 80, respawn 3000).
- resolveFrame é async (await getCharacterConfig): SEMPRE revalidar pós-await (target vivo/não sentado/presente + atacante vivo/não sentado) — dois atacantes paralelos duplicavam player_died/timers de revive. Timer de revive carrega token (`reviveAt`) e só revive se `deadUntil.get() === token`.
...
## Níveis por item de arma (ago/2026)
- Levels por VARIANTE (item) vivem em `variants` dentro do jsonb de weapon_families — sem tabela nova; ausência do campo = compat total (linhas antigas intactas). Associação de família e níveis são campos independentes do MESMO registro: todo caller de families.save precisa preservar `variants` (o handleAssociateFamily já quase apagou).
- Regras: levels contíguos 1..N (level 1 fixo), damage int 0..1000, speed 0.1..10 (multiplicador; 1 = 12fps padrão do jogo). Resolução de dano: level do item → damagePerHit do perfil → 10.
- Runtime ainda NÃO consome níveis (armas são assadas no spritesheet gerado; runtime só conhece characterId; combate vivo usa o sistema legado de personagem). Quando equipamento existir: cliente aplica speed no anims.timeScale do ataque; servidor resolve dano pelo repositório. Helpers prontos: resolveWeaponLevelStats (shared) e resolveWeaponItemStats (weaponLoader).
- Lição do review: neste editor (save imediato + rascunho local), rascunho keyed precisa ser limpo em save OK e rebase em recarga (padrão epoch via prop) — senão rascunho velho sobrescreve mudanças do servidor. Inputs numéricos controlados: manter string crua por campo e commitar só número válido (Number, não parseInt).

## Craft admin (round craft, ago/2026)
- Regra: novas categorias "tipo arma" (crafttools) NÃO ganham sistema próprio — entram em `WEAPON_LIKE_CATEGORIES` e reutilizam tabelas/painéis de weapon. **Why:** famílias são por nome de arquivo (sem colisão entre pastas) e o admin já resolve hitbox/níveis por família.
...
## Sistema de personagem do jogador (ago/2026): identidade de sala + aparência composta
- **Identidade de sala é do TOKEN, nunca do cliente**: WorldRoom.onJoin verifica o JWT do Supabase (`verifySupabaseToken` em auth/supabaseAuth.ts, compartilhado com o middleware HTTP); `options.playerId` é ignorado (era spoofável — com a persistência de equipamento isso virou escrita cross-conta na tabela da vítima, achado de review). Sem token = `anon:<sessionId>` (nunca casa com UUID → não lê/escreve personagem nem derruba sessão alheia). Toda sala nova segue o padrão; o cliente manda `token` nas options de join (world E arena), lido de supabase.auth.getSession() na hora do join (auto-refresh cobre expiração).
- **Deploy client+server é atômico**: cliente novo esconde remotes sem `appearance` (legados fora do jogo por design) — contra servidor cloud velho ninguém renderiza. Nunca pushar metade.
- **Texturas compostas exigem lifecycle explícito**: cada receita única (`pc-<hash>`) retém canvas 2208×384 ≈3MB p/ sempre; `pruneComposedAppearances(scene, inUse)` roda em leave/destroyAll/troca local/troca remota. Builds em voo protegidos por: mapa inFlight (dedupe), stamps `pendingAppearanceAdds` (remotes) e `localAppearanceTargetId` marcado ANTES do await (senão prune disparado por leave coleta o def sendo construído). Reoferta de join com receita nova re-estampa o pending e o job velho aborta no check do stamp (corrida: equipou durante a composição inicial → sprite congelava na receita velha).
- **Todo handler de sala com await no meio precisa de seq map last-request-wins** (`equipSeq` agora; mesma lição do set_character jul/2026 — segunda ocorrência do padrão).
- **VITE_COLYSEUS_LOCAL=1 RE-ADICIONADO** (ago/2026; corrige a nota de jul que dizia "removido do dev env") para testar o sistema de personagem antes do push — o cloud roda código velho sem os handlers novos. Seguro: o coordenador local se auto-bloqueia sob REPL_ID (log "[Coordinator] Skipped"). Voltar ao modo cloud = deletar a env var de development. Cliente colyseus.js 0.15 decodifica o estado do api-server local OK (evidência e2e: entrou no mundo local, "1 online" no HUD, sem overlay de estado inválido).
- **Tester headless × WebGL é FLAKY, não absoluto** (refina nota jul/2026): desta vez entrou no mundo em 1280×800 e completou o fluxo do modal; resize p/ 402×874 + reload esgotou contextos ("Cannot create WebGL context"). Phaser 4 NÃO tem fallback Canvas — `type: Phaser.AUTO` não ajuda (tentado e revertido). Checagem visual mobile = aparelho real do usuário; layout do modal conferido por classes (grid-cols-1 → sm:grid-cols-2, max-h-[92vh] overflow-y-auto, thumbs 52px).
- **Fluxo de criação verificado por e2e**: signup → modal obrigatório não-fechável → classe → aparência (preview/thumbs/tons/girar) → PUT /api/me/character 503 exibindo o bloco SQL dentro do modal enquanto `player_characters` não existe (UX projetada para o admin rodar o SQL).

## Mundo de Coleta (CraftingWorld) — lições do port do mapa Tiled
- Tilesets EXTERNOS (.tsx): Phaser não lê; embutir via plugin Vite que parseia o XML (regex tolerante à ordem de atributos), serve TMJ embutido em rota dev e emite no build (generateBundle). Tilesets sem imagem no repo: descartar + REMOVER os objetos que os usam, senão gid órfão quebra o parse do Phaser.
- Armadilha: nomes de tileset duplicados (mapa referenciava o mesmo tileset por 2 caminhos). Phaser casa addTilesetImage por NOME → deduplicar nomes no embed ("x #2") e compartilhar textura por URL de imagem.
- Y-sort barato num mapa só: banda de depth (100..189 por Y) entre chão (0) e camadas above (200), aplicada no update() apenas com runtime ativo; remotos são re-ordenados no mesmo loop. Volta ao mapa principal é segura porque troca de sala destrói/recria remotos com depth padrão.
...
## Armas finais: arco/flecha + scanner de materiais (set/2026)
- Scanner do gerador: categorias aceitam 1 nível de subpasta; materiais são conjunto FECHADO (`MATERIAL_VARIANT_ORDER`, sync manual com `WEAPON_MATERIAL_IDS` no WeaponShapes espelhado ×3); alias de pasta `weapons/`→categoria `weapon` (URLs preservam caminho físico); material fora do conjunto vira família própria; `_cN` mantém precedência; wood promovido a default SEM warning (sem PNG base por design).
- Pareamento arco↔flecha: `family.group` (= subpasta) é o ÚNICO vínculo — projétil = família `arrow` com o mesmo group. A UI de aparência esconde projéteis via `manifestWithoutProjectiles`, mas o manifest CRU precisa ficar no estado para o pareamento.
- **Contrato central**: dano/alcance/hitbox do tiro são autorados na FAMÍLIA DA FLECHA, chaveados pelo variantId (material) do ARCO — admin salva lá e o runtime TEM que resolver `resolveWeaponShootStats(arrowFam.id, materialDoArco)`. Review pegou o runtime lendo da família do arco; defaults mascaram a regressão (tudo "funciona" com valores errados).
...
### Desempenho do cliente — rodada 2 (movimento/render, set 2026)
- Remotos: interpolação com delay 120ms e catch-up exponencial POR TEMPO (tau 90ms, `getPosition(deltaMs)`); NUNCA aplicar Math.floor na posição interpolada (floor em px de mundo = saltos de 3-4px de tela em zoom 3-4; `roundPixels` já quantiza em tela). Anim walk/idle usa histerese `stillFrames` (<8 frames = ainda andando), não só o flag `isMoving` do estado.
- A*: buffers (gScore/fScore/parent/closed) pré-alocados no buildGrid e resetados com fill() — alocar ~1,2MB por findPath causava rajadas de GC (engasgos no hold-to-steer a cada 150ms). Atalho de linha-de-visada antes do A* (só quando o destino NÃO foi realocado pelo findNearestUnblocked). findPath devolver só 2 pontos é contrato válido.
- Hold-to-steer: zona de desaceleração do último waypoint é PULADA durante hold (senão vira "arrancada" ao virar o polegar) + zona morta de 20px (dedo sobre o personagem = stopMovement, não path até si mesmo).
- Filtro de textura dinâmico por zoom: LINEAR se zoom<0.75 (NEAREST em minificação decima pixels), NEAREST senão; o método reaplica quando a CONTAGEM de texturas muda — texturas carregadas depois (switchMap/aparências) chegam com setFilter(NEAREST) explícito e ficariam de fora.
- CORREÇÃO pós-publicação: NUNCA disparar varredura de setFilter por CONTAGEM de texturas — objetos Text (avisos/dano) criam/destroem texturas canvas o tempo todo em combate; varrer ~300 texturas com GL a cada golpe derrubou o FPS em produção. Padrão certo: flag dirty setada em `this.load.on('complete')` (dispara depois dos setFilter explícitos do carregamento) + edge-trigger no limiar de zoom.
- Hold-to-steer é DIREÇÃO, não destino: alvo clampado a ~240px à frente do corpo (rota curta = A*/LOS baratos; rota longa a cada 150ms custava 10-30ms no celular → hitch → passo de compensação do Matter = "pique"). Com clamp, repath imediato quando target=null (senão micro-pausa a cada chegada no ponto intermediário). Runner do Matter (Phaser 4) já suaviza/clampa delta (maxFrameTime ~33ms) — não mexer.

## Estações de criação (/admin/stations)
- Config da estação (abas+rows) vive em jsonb salvo como documento COMPLETO por PUT → saves otimistas exigem fila POR ESTAÇÃO com "latest wins" (refs desired/saved + inFlight; sem loadAll automático no erro). Vínculo item→estação idem: fila POR ITEM + sequência, revert só se a op ainda é a mais nova. **Why:** PUTs concorrentes fora de ordem deixavam config antiga como valor final no banco (achado de review).
- StationPreview é o espelho compartilhado do painel do jogo (admin passa `edit` + inventário simulado por hash; o jogo renderiza sem `edit` com inventário real). Timer do loader fictício precisa ser cancelado em QUALQUER troca de contexto (aba/estação/seleção), não só no unmount.
- Integração no jogo pendente: usuário ainda marca os pontos de contato no Tiled; rota pública cacheada GET /api/craft-stations-data já pronta para o cliente do jogo.

## Inventário / equipamento (rodada de refação, set 2026)
- Dev preview fala com o api-server LOCAL (`VITE_COLYSEUS_LOCAL=1`, workflow "API Server" precisa estar de pé). A nuvem Colyseus só atualiza com push REAL no GitHub — `main` ficar à frente de `origin/main` = produção velha (sintomas "antigos" que já estão corrigidos no código).
- Arma "não aparecia" no personagem: `asset_categories` guardava refs de família que o manifest do gerador RENOMEOU (ex.: `sword1`) → ref órfã, aparência sem arma. **Regra:** ao renomear famílias no manifest, migrar `asset_categories` e `player_characters.equipped_weapon` juntos; refs de arma no banco devem ser completas (`gen:weapon/<família>/<variante>`). Cache de categorias no servidor: 30s.
- Cliente equipa a arma da classe enviando `equip_weapon {equip:true}` SEM ref — o servidor resolve a arma da classe (`findClassWeaponRef`) e devolve `equip_error {message}` nas recusas. Não voltar a resolver a ref no cliente.
- Phaser 4: mundo→tela = `cameras.main.matrixCombined.transformPoint(x,y)` (inverso exato de `getWorldPoint`), depois × escala `rect/canvas.width` + `rect.left/top`; tela→mundo divide pela mesma escala. Fórmula manual com zoom/rotação estava errada.
...
## Inventário: DnD, durabilidade e miniaturas (set/2026)
- Ghost de arrasto precisa de `z-index` acima do painel do inventário (`fixed z-[500]`) — com z menor o item "não acompanha o cursor" porque fica atrás do painel. Ghost em `fixed z-[1000]`, movido direto no DOM (`translate3d`) sem re-render por pointermove; célula de origem faz `setPointerCapture` e `lostpointercapture`/`blur` cancelam.
- Swap animado = FLIP com WAAPI (`element.animate`) medindo os wrappers `data-flip-key`; o store dispara um sinal síncrono ANTES do `set` (slotChangeSignal) para cada superfície (painel + hotbar espelhada) medir os rects "antes". Sem esse sinal a segunda superfície não tem o "first" e pula.
- Durabilidade de ferramentas: coluna nullable `collection_inventory.durability` (null = cheia) por pilha usuário×itemKey; PostgREST não faz DDL, então o servidor tolera a coluna ausente (`42703` no select, `PGRST204` no upsert; cache 60 s) e devolve `durabilityColumnMissing` + SQL — cliente esconde as barras e mostra o banner. O usuário precisa rodar o ALTER no Supabase; até lá o caminho feliz de `/tool-wear` não é testável localmente.
- Modelo de confiança: `/collect` e `/tool-wear` são "cliente reporta, servidor persiste e aplica a regra" (não há simulação do mundo de coleta no servidor). Reforço escolhido: lotes de desgaste idempotentes por `requestId` (memória por processo, TTL 10 min; falhas não são lembradas) e a fila do cliente re-tenta o MESMO lote/id. Não prometer "anti-cheat" nesse endpoint sem mover os golpes para o servidor.
- Folhas de sprite das picaretas não têm pose parada (só colunas de golpe) → miniatura em branco; `visibleFrameCol` procura o primeiro frame com pixel opaco na linha. Reaparece em qualquer nova ferramenta desenhada só com frames de ação.
- Testes vitest que importam a árvore do inventário precisam de `vi.mock('../../config/colyseus')` (o módulo lê `window` na importação com `VITE_COLYSEUS_LOCAL=1`); `vitest.config.ts` usa `esbuild.jsx: 'automatic'` para JSX em `.test.ts(x)`.
- Scripts Node ad hoc com `@supabase/supabase-js` devem ficar DENTRO de `artifacts/api-server` (resolução do pnpm); em `/tmp` dá ERR_MODULE_NOT_FOUND. Usuário temporário via `auth.admin.createUser` + `signInWithPassword`, apagado no `finally`.
- Node 24 `--experimental-strip-types` roda `.ts` do servidor sem tsx, mas rejeita parameter properties (`constructor(private x)`) — escrever campos explícitos.

## Controles de teclado (WASD/atacar/interagir remapeáveis, set/2026)
- Teclas identificadas por `KeyboardEvent.code` (tecla física — WASD igual em qualquer layout) e exibidas pelo **rótulo capturado de `key`** (ABNT2 mostra "Ç", não "Semicolon"); com Shift segurado rotula pela tecla base (Shift+3 é "3"). Ouvintes DOM na `window`, não o plugin de teclado do Phaser (não expõe `code`).
- **Por quê Ctrl/Alt/Meta estão fora** (nem como tecla, nem em combinação): Ctrl+F/Ctrl+W/Alt+… são atalhos do navegador; mapear só por `code` os capturaria. Shift é permitido. Tab/F-keys/Esc/Meta proibidos.
- Conflito ao religar = **troca** entre as duas ações (nenhuma fica sem tecla); o saneamento do storage nunca deixa duas ações na mesma tecla (reserva de teclas livres). Persistência por aparelho (localStorage), não por perfil.
- Teclado tem **prioridade sobre o clique/hold-to-move** (cancela o caminho sem parar); quando as travas da cena engajam (partida, assento, troca de mapa, movementLocked) solta as teclas presas e emite parada. Ignorado quando há campo de texto focado, o modal de Configurações está aberto ou a aba Controles está capturando. Também `preventDefault` no **keyup** das teclas do jogo (Espaço/Enter "clicariam" o botão do HUD que ficou com foco).
- Verificação sem WebGL: bancada DEV `/dev/controles` (modal real + leitura ao vivo do vetor/ações). Conta de teste do repo (scripts e2e) entra no jogo, mas "Enter World" segue morrendo sem WebGL no navegador do testador.

## Estações portáteis posicionáveis (bigorna/mesa/fornalha/poções, set/2026)
- Modelo: item built-in de craft com durabilidade (max no admin); posicionado vira `PlacedStationState` no MapSchema da sala (só salas `craft:`), dono/autorizados craftam via `craft_item {placedId}` (−1 durabilidade), 5 min depois vira drop comum COM durabilidade. Toda geometria/regra pura em `shared/craft/PlaceableStations.ts` (espelhado servidor↔cliente); âncora = centro da base do CORPO, sprite deslocado por `placedStationSpriteOffset`.
- Durabilidade viaja com a cópia: `takeWithDurability`/`giveWithDurability` (merge "pior das duas"); a pilha que fica volta a "cheia" (null). Por isso o cap é **1 cópia por inventário** (`PLACEABLE_STACK_LIMIT`) — sem isso a durabilidade por cópia não tem dono. O cap é conferido DENTRO de `applyInventoryDeltas` (fila serializada do usuário); a checagem prévia no craftService é só mensagem bonita (review pegou a corrida).
- Posicionar roda sob UM lock da sala (`placed:place`): validar → debitar → inserir; sem isso dois jogadores sobrepõem estações. Craft/recolher/varredura usam lock por estação (`placed:<id>`).
- **Fail-closed sem a coluna `collection_inventory.durability`**: `takeWithDurability` devolve `durabilityPersisted=false` → posicionar é recusado (devolve a cópia) com mensagem pedindo a migração. Motivo: sem a coluna, recolher+posicionar renovaria a durabilidade infinitamente. Ferramentas/drops continuam tolerando a coluna ausente.
- colyseus.js 0.15 + @colyseus/schema 2.0.x: `MapSchema.onAdd(cb)` tem `triggerAll=true` por padrão (replay dos itens já decodificados) — NÃO precisa iterar o mapa antes; um review sugeriu isso como bug (falso positivo). Deploys antigos sem o campo: guardar `typeof map?.onAdd === 'function'`.
- Cliente: colisão exata = corpo Matter estático + camada dinâmica na grade do A* (`setDynamicRects`, inflate 12) — animais desviam sozinhos; validação local de paredes usa `isMapRectBlocked` (só colisões do mapa). Clicar na estação não anda (pointerup faz `hitTest` antes). Spritesheets carregadas sob demanda pelo loader da cena (`load.start()` fora do preload funciona no Phaser 4; tratar `loaderror`).
- Verificação local sem WebGL continua impossível para o fluxo no mapa (posicionar/recolher/permissão) — só tsc, vitest dos helpers e a página /admin/craft foram conferidos; o resto depende do usuário testar no jogo.
- Locks `.git/*.lock` órfãos (0 bytes, sem processo git) apareceram durante `git reset --hard` logo após o push via API; esperar ~30 s e remover manualmente resolve.

## Energia, habilidades (XP/níveis) e badges de craft (set/2026)
- Espelho `server/src` ↔ `artifacts/api-server/src/src` é `cp` verbatim; o shared do CLIENTE (`artifacts/chessworld/src/shared`) também é cópia verbatim e MANTÉM o sufixo `.js` nos imports (Vite resolve) — nunca passar sed neles; conferir com `diff`.
- NUNCA `rm -rf artifacts/chessworld/src/shared` antes de copiar: os `*.test.ts` do shared existem SÓ no cliente (vitest roda lá; 232 testes com eles, 85 sem). Copiar por cima (`cp -r server/src/shared/. artifacts/chessworld/src/shared/`) e conferir `diff -rq ... | grep -v '\.test\.ts'` vazio. Se sumirem, `git checkout --` neles. O `rm -rf` do api-server (`src/src`) continua ok.
- Ids de item de craft: built-ins são `familia/variante` e gerados são `gen:categoria/familia/variante` — validar SEMPRE com `classifyCraftEntityId` (regex ingênua rejeita os `gen:`).
- Config de energia/skills é UM documento jsonb (linha única `energy_skills_config`) salvo inteiro pelo admin; o parser preenche defaults (inclusive mapas por recurso de mining/woodcutting) para o doc salvo ser explícito. Progresso por jogador em `player_progress`; tabelas ausentes toleradas (memória + banner SQL no admin) — mesmo padrão do inventário.
- Modelo de confiança repete o do `/collect`: golpes/nós são reportados pelo cliente em lotes com `requestId` obrigatório+idempotente; o serviço aplica teto por minuto por jogador (240 golpes, 60 nós) e descarta o excesso. Review arquiteto marca isso como "severo" toda vez — a resposta é a mesma: o mapa de coleta não roda no servidor; não prometer anti-cheat.
- Decisões de jogo: energia NUNCA bloqueia ação (só fome/fraco/morte por limiares); custos "X a cada N" com contadores persistidos; craft cobra energia/XP × quantidade; comer consome só o necessário até encher (energia cheia → erro); revive enche energia; morte por fome usa `killPlayer(sessionId,'Fome')`.
- Badges de comida: `food` = item de culinária (ingrediente OU prato) → XP de Cooking por unidade obtida (craft × qtd em QUALQUER estação, ou coleta do chão), um só mapa `skills.cooking.items`; `edible` = pode ser comido (energia). `food` sozinha NÃO come — o usuário pediu explicitamente. A aba "Cozinhar" da fornalha não decide mais nada (`isCookingTab` removido).
- Nome das skills é config (`skills.names`, editável clicando no título no admin); no cliente usar `skillName(config.skills, id)`, nunca `SKILL_LABELS` direto (o ticker/HUD passa a mostrar o nome renomeado).
- Painéis do HUD (inventário, habilidades) dividem o canto inferior direito e o Esc → exclusivos (abrir um fecha o outro, coordenado no progressStore). Bancada `/dev/inventario?skills=1` abre o painel com snapshot fictício.
- `index.css` força `overflow:hidden` em html/body/#root: toda página não-jogo precisa de `useDocumentScrollUnlock()` (as admin antigas têm o useEffect copiado; as novas usam o hook), senão não rola.
- Admin de badges salva por item na hora com fila POR ITEM + sequência (mesmo padrão das estações) — só sequência não basta: a requisição antiga pode chegar por último e o servidor faz delete/upsert incondicional.
- `/admin/skills-energy#skills` abre direto na aba Skills (hash = aba inicial); útil para screenshots sem clique.
- Armadilha "tabela criada, linha ainda não": repositórios de doc único devolvem `config: null` quando não há linha (não é `tableMissing`). Rotas admin devem responder os DEFAULTS + `saved:false`, e a página normalizar com o parser — `setConfig(null)` derrubou a página inteira (crash em `config.skills`). Testar SEMPRE o estado "tabela vazia" além de "tabela ausente".

## Snapshot de progresso por dois caminhos + comida voando (set/2026)
- O MESMO `ProgressSnapshot` chega ao cliente pela resposta HTTP do lote de atividade E por `progress_update` da sala (mesmo processo), e re-tentativas idempotentes o repetem → "+5 XP" aparecia em dobro. Solução: `seq` crescente por jogador no snapshot (só memória, ancorado em `Date.now()` — sobrevive a restart sem persistir) e o cliente só aplica o mais novo (`lib/progress/snapshotOrder.ts`). Qualquer caminho novo que entregue snapshot precisa passar por `applySnapshot` (não duplicar ticks à mão); snapshots falsos da bancada precisam de `seq`.
- Comer: a previsão de quantas unidades voam usa a MESMA regra do servidor (`min(possui, ceil(faltando/energiaPorUnidade))`); se a regra mudar no `ProgressService.eat`, mudar `predictEatCount` junto. A pilha em mastigação fica sem arrasto até o servidor responder (drop no meio faria a sala recusar).
- Bancada `/dev/inventario` agora é offline de verdade para comer: `primeCraftData()` injeta item+badges sem rede e uma sala falsa (`setEatSender`) responde com a regra do servidor. É o lugar para testar qualquer efeito da hotbar sem WebGL; medir tempo de animação com amostrador `setInterval` dentro da página (screenshots do testador atrasam segundos e enganam).
