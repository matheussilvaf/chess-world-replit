...
- Mutação de contrato/recompensa usa CAS via PostgREST (`update().eq(user_id).is('active', null)` ou `.eq('active->>contractId', id)` + `select()`; 0 linhas = já processado). Reutilizar o padrão para qualquer "1 ativo por jogador" com recompensa.
